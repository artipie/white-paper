...
\begin{tikzpicture}[node distance = 3]
  \node[block] (engine) {Artipie engine};
  \node[block, below=of engine] (repo) {Repository};
  \node[block, right=of repo] (asto) {Abstract storage};
  \node[block, left=of engine] (ext) {Extensions};
  \node[block, above=of asto] (adapter) {Adapter};
  \path[ln]
    (ext) edge node {uses HTTP API} (engine)
    (engine) edge node {manage} (repo)
    (repo) edge[bend left=10, above] node {upload artifacts} (asto)
    (asto) edge[bend left=10, below] node {download artifacts} (repo)
    (adapter) edge node {updates meta-information} (asto)
    (repo) edge node {triggers} (adapter);
\end{tikzpicture}
